...
async function loadCategories(start, end) {
  let query =
    "SELECT * FROM " +
    CONSTANTS.tables.category +
    " cat ORDER BY cat.id LIMIT ?, ?";
...
async function getCategoryMenuArray() {
  ...
  let error;
  try {
    categories = await loadCategories(0, 200); //assuming 200 categoeries is all that will ever be added if even that
  } catch (ex) {
    error = ex;
  }
  ...
  categories.forEach(cat => {
    if (cat.parent) {
      catObject[cat.parent].children.push(catObject[cat.id]);
    }
  });
