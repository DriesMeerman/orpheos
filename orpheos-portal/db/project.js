...
async function getProjectByIdOld(id) {
  let query =
    "SELECT * FROM " +
    CONSTANTS.tables.project +
    " proj WHERE id = " +
    id +
    " ORDER BY proj.id LIMIT 1";
...
let findProjectsByName = makeGetFunction(
  "SELECT * FROM " +
    CONSTANTS.tables.project +
    " project WHERE project.name LIKE CONCAT('%', ?,  '%')"
);
let findProjectsByCategory = makeGetFunction(
  "SELECT * FROM " + CONSTANTS.tables.project + " project WHERE category = ?"
);
let findProjectsByOwner = makeGetFunction(
  "SELECT * FROM " + CONSTANTS.tables.project + " project WHERE owner = ?"
);
let getAllProjectsRaw = makeGetFunction(
  "SELECT * FROM " + CONSTANTS.tables.project + " project LIMIT ?,?"
);
